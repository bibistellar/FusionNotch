...
final class ProcessMonitoringCoordinator {
    ...
    func reconcileSessionAttachments(
        ...
        let mergedSessions = mergedWithSyntheticCodexSessions(
    ...
    // MARK: - Synthetic Codex sessions

    /// Upstream synthesizes idle sessions for Claude and Cursor only, so a Codex process
    /// that hasn't fired a hook yet showed up as nothing at all — leaving the panel
    /// looking like every idle agent is Claude. Codex is easy to do properly: discovery
    /// reads the session UUID out of the rollout transcript path, and that is the same ID
    /// the Codex hooks report, so an ID match is enough to dedupe against a real session.
    func mergedWithSyntheticCodexSessions(
        existingSessions: [AgentSession],
        activeProcesses: [ActiveProcessSnapshot],
        now: Date = .now
    ) -> [AgentSession] {
        let activeCodexProcesses = activeProcesses.filter { $0.tool == .codex }
        guard !activeCodexProcesses.isEmpty else { return existingSessions }

        var seenSessionIDs = Set(existingSessions.map(\.id))
        var syntheticSessions: [AgentSession] = []

        for process in activeCodexProcesses.sorted(by: { processIdentityKey($0) < processIdentityKey($1) }) {
            guard let sessionID = process.sessionID,
                  seenSessionIDs.insert(sessionID).inserted else { continue }
            syntheticSessions.append(syntheticCodexSession(for: process, sessionID: sessionID, now: now))
        }

        return existingSessions + syntheticSessions
    }

    private func syntheticCodexSession(
        for process: ActiveProcessSnapshot,
        sessionID: String,
        now: Date
    ) -> AgentSession {
        ...
        var session = AgentSession(
            ...
            attachmentState: .attached,
            phase: .completed,
            summary: "Codex session detected from \(terminalApp).",
            updatedAt: now,
            jumpTarget: JumpTarget(
                terminalApp: terminalApp,
                workspaceName: workspaceName,
